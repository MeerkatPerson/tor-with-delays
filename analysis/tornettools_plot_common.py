from tornettools_util import which
...
class TailLog(mscale.ScaleBase):
    ...
    def set_default_locators_and_formatters(self, axis):
        ...
        minloc = [10**(-1 * self.nines) * k for k in range(100)
...
def draw_cdf_ci(axis, dataset, confidence=0.95, yscale=None, **kwargs):
    ...
    if total_num_items == 0:
        ...
        x, x_min, x_max = __compute_sample_mean_and_error(
            bucket_list, confidence)
...
def log_stats(filename, msg, dist):
    ...
    with open(filename, 'a') as outf:
        print(msg, file=outf)
        print("min={} q1={} median={} q3={} max={} mean={} stddev={}".format(min(
            b), score(b, 25), median(b), score(b, 75), max(b), mean(b), std(b)), file=outf)
