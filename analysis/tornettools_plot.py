...
import tornettools_plot_common
...
def run(args):
    ...
    tornettools_plot_common.set_plot_options()
...
def __plot_tornet(args):
    ...
    torperf_dbs = load_torperf_datasets(args.tor_metrics_path)
    ...
    for circuittype in ('exit', 'onionservice'):
        ...
        tornet_dbs = __load_tornet_datasets(args, __pattern_for_basename(
            circuittype, "perfclient_circuit_build_time"))
        ...
        tornet_dbs = __load_tornet_datasets(
            args, __pattern_for_basename(circuittype, 'round_trip_time'))
        ...
        tornet_dbs = __load_tornet_datasets(
            args, __pattern_for_basename(circuittype, 'time_to_last_byte_recv'))
        logging.info("Plotting transfer times")
        __plot_transfer_time(
            args, circuittype, torperf_dbs, tornet_dbs, "51200")
        __plot_transfer_time(
            args, circuittype, torperf_dbs, tornet_dbs, "1048576")
        __plot_transfer_time(
            args, circuittype, torperf_dbs, tornet_dbs, "5242880")

        logging.info(f"Loading {circuittype} tornet goodput data")
        tornet_dbs = __load_tornet_datasets(
            args, __pattern_for_basename(circuittype, 'perfclient_goodput'))
        ...
        tornet_dbs = __load_tornet_datasets(
            args, __pattern_for_basename(circuittype, 'perfclient_goodput_5MiB'))
        ...
        tornet_dbs = __load_tornet_datasets(
            args, __pattern_for_basename(circuittype, 'error_rate'))
        logging.info(f"Plotting {circuittype} transfer error rates")
        __plot_transfer_error_rates(
            args, circuittype, torperf_dbs, tornet_dbs, "ALL")
...
def __plot_relay_goodput(args, torperf_dbs, tornet_dbs, net_scale):
    ...
    plot_cdf_figure(args, dbs_to_plot, 'relay_goodput',
...
def __plot_circuit_build_time(args, circuittype, torperf_dbs, tornet_dbs):
    ...
    plot_cdf_figure(args, dbs_to_plot, f'circuit_build_time.{circuittype}',
                    yscale="taillog",
...
def __plot_round_trip_time(args, circuittype, torperf_dbs, tornet_dbs):
    ...
    plot_cdf_figure(args, dbs_to_plot, f'round_trip_time.{circuittype}',
                    yscale="taillog",
...
def __plot_transfer_time(args, circuittype, torperf_dbs, tornet_dbs, bytes_key):
    ...
    # cache the corresponding data in the 'data' keyword for __plot_cdf_figure
    for tornet_db in tornet_dbs:
        tornet_db['data'] = [tornet_db['dataset'][i][bytes_key] for i, _ in enumerate(
            tornet_db['dataset']) if bytes_key in tornet_db['dataset'][i]]
    for torperf_db in torperf_dbs:
        torperf_db['data'] = [torperf_db['dataset']
                              ['download_times'][bytes_key]]

    dbs_to_plot = torperf_dbs + tornet_dbs

    plot_cdf_figure(args, dbs_to_plot, f"transfer_time_{bytes_key}.{circuittype}",
                    yscale="taillog",
...
def __plot_transfer_error_rates(args, circuittype, torperf_dbs, tornet_dbs, error_key):
    ...
    # cache the corresponding data in the 'data' keyword for __plot_cdf_figure
    for tornet_db in tornet_dbs:
        tornet_db['data'] = [tornet_db['dataset'][i][error_key] for i, _ in enumerate(
            tornet_db['dataset']) if error_key in tornet_db['dataset'][i]]
    for torperf_db in torperf_dbs:
        err_rates = __compute_torperf_error_rates(
            torperf_db['dataset']['daily_counts'])
        torperf_db['data'] = [err_rates]

    dbs_to_plot = torperf_dbs + tornet_dbs

    plot_cdf_figure(args, dbs_to_plot, f"transfer_error_rates_{error_key}.{circuittype}",
...
def __plot_client_goodput(args, circuittype, torperf_dbs, tornet_dbs):
    ...
    for tornet_db in tornet_dbs:
        ...
        tornet_db['data'] = [[x * 2**20 / 1e6 for x in ds]
                             for ds in tornet_db['dataset']]
    for torperf_db in torperf_dbs:
        # Covert to Mbps
        client_gput = [
            t / 1e6 for t in torperf_db['dataset']["client_goodput"]]
        torperf_db['data'] = [client_gput]

    dbs_to_plot = torperf_dbs + tornet_dbs

    plot_cdf_figure(args, dbs_to_plot, f'client_goodput.{circuittype}',
                    yscale="taillog",
...
def __plot_client_goodput_5MiB(args, circuittype, torperf_dbs, tornet_dbs):
    ...
    for torperf_db in torperf_dbs:
        # Covert to Mbps
        client_gput = [t / 1e6 for t in torperf_db['dataset']
                       ["client_goodput_5MiB"]]
        torperf_db['data'] = [client_gput]

    dbs_to_plot = torperf_dbs + tornet_dbs

    plot_cdf_figure(args, dbs_to_plot, f'client_goodput_5MiB.{circuittype}',
                    yscale="taillog",
                    xlabel=f"{circuittype} Client Transfer Goodput (Mbit/s): 4 to 5 MiB")


def plot_cdf_figure(args, dbs, filename, xscale=None, yscale=None, xlabel=None, ylabel="CDF"):
    color_cycle = cycle(tornettools_plot_common.DEFAULT_COLORS)
    linestyle_cycle = cycle(tornettools_plot_common.DEFAULT_LINESTYLES)
    ...
    for db in dbs:
        if 'data' not in db or len(db['data']) < 1:
            ...
            (plot_func, d) = tornettools_plot_common.draw_cdf, db['data'][0]
        else:
            (plot_func, d) = tornettools_plot_common.draw_cdf_ci, db['data']
    ...
    for db in dbs:
        if len(db['data']) >= 1 and len(db['data'][0]) >= 1:
            q = tornettools_plot_common.quantile(db['data'][0], 0.99)
            x_visible_max = q if x_visible_max is None else max(
                x_visible_max, q)
...
def __plot_timeseries_figure(args, dbs, filename, xtime=False, ytime=False, xlabel=None, ylabel=None):
    ...
    for db in dbs:
        ...
        if len(db['dataset']) > 1:
            plot_func = tornettools_plot_common.draw_line_ci
        else:
            plot_func = tornettools_plot_common.draw_line
...
def __plot_finish(args, lines, labels, filename):
    ...
    pyplot.tick_params(axis='both', which='minor', labelsize=7)
    pyplot.grid(True, axis='both', which='minor',
                color='0.1', linestyle=':', linewidth='0.5')
    pyplot.grid(True, axis='both', which='major',
                color='0.1', linestyle=':', linewidth='1.0')

    pyplot.legend(lines, labels, loc='best', prop={'size': 18})
    pyplot.tight_layout(pad=0.3)

    if args is not None:
        pyplot.savefig(
            f"{args.prefix}/{filename}.{'png' if args.plot_pngs else 'pdf'}")
        args.pdfpages.savefig()
    else:
        pyplot.savefig(f"plots/{filename}.pdf")
...
def __load_tornet_datasets(args, filepattern):
    ...
    if args.tornet_collection_path is not None:
        for collection_dir in args.tornet_collection_path:
            tornet_db = {
                'dataset': [tornettools_util.load_json_data(p) for p in tornettools_util.find_matching_files_in_dir(collection_dir, filepattern)],
...
def load_torperf_datasets(torperf_argset):
    torperf_dbs = []

    if torperf_argset is not None:
        for torperf_args in torperf_argset:
            torperf_db = {
                'dataset': tornettools_util.load_json_data(torperf_args[0]) if torperf_args[0] is not None else None,
            ...
            if 'net_scale' in d:
                if net_scale == 0.0:
                    ...
                    if float(d['net_scale']) != net_scale:
                        logging.warning(
                            "Some of your tornet data is from networks of different scale")
                        logging.critical(
                            f"Found network scales {net_scale} and {float(d['net_scale'])} and they don't match")
