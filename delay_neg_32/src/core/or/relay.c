...
// Added additional libraries for being able to add timed callback to introduce delay in 
// append_cell_to_circuit_queue()
#include "lib/time/compat_time.h"   // for struct timeval
#include "lib/defs/time.h"          // for TOR_USEC_PER_SEC
#include "lib/evloop/timers.h"      // for struct tor_timer_t
#include "lib/malloc/malloc.h"      // for tor_malloc and tor_free
#include "core/or/circuitmux.h"     // for cmux_make_circuit_{active, inactive}

#include "core/or/circuitdelay.h"  // 8/12 for delay negotiation

// import math functionality for sampling random delay from exponential distribution (19/11)
#include <math.h>
#include "lib/math/prob_distr.h"

// add crypto library for generating random seed for sampling from exp. distr. (20/11)
#include "lib/crypt_ops/crypto_rand.h"
                           ...
                           cell_direction_t cell_direction)
{
  ...
  // 17/11 replace call to 'append_cell_to_circuit_queue' with call to our
  // custom function
  append_cell_to_circuit_delay_queue(circ, chan, cell, cell_direction, 0);
  return 0;
}
                           ...
                           const char *filename, int lineno))
{
  ...
  // 17/11 replace call to 'append_cell_to_circuit_queue' with call to our
  // custom function which introduces a delay
  append_cell_to_circuit_delay_queue(circ, chan, cell, cell_direction, on_stream);
  return 0;
}
                               ...
                               const char *filename, int lineno))
{
  ...
  if (cell_direction == CELL_DIRECTION_OUT) {
    ...
    log_info(LD_GENERAL,"Attempting to send relay cell with command %d from origin circuit.", relay_command);

    /**
     * 21/12 trigger delay re-negotiation if origin_circ->relay_cell_count is 0;
     * increment it mod 32
     */

    origin_circ->relay_cell_count = ((origin_circ->relay_cell_count + 1) % 32);

    if (((origin_circ->relay_cell_count)-1) == 0) {

      log_info(LD_GENERAL,"Relay cell threshold at origin reached.");

      circdelay_event_cell_threshold_reached(origin_circ);
    }

    log_info(LD_GENERAL,"Incremented relay cell threshold at origin, it is now %d", origin_circ->relay_cell_count);
                     ...
                     relay_header_t *rh, int optimistic_data)
{
  ...
  /** 8/12 Also check if it is a cell destined for the circuit delay subsystem. */
  if (circdelay_check_received_cell(cell, circ, layer_hint, rh) == 0) {
    log_debug(domain, "Cell handled as circuit delay control cell");
    return 0;
  }
...
/**
 * Callback function for marking circuit as active again after
 * delay has elapsed (it is marked inactive at the end of
 * append_cell_to_circuit_queue to simulate a delay as a strategy
 * for thwarting traffic analysis based on timing patterns)
 * 
 * We can't do with just one argument, so
 * need to use a helper struct that encapsulates everything we need.
 */

static void
dequeue_from_delay_queue_callback(tor_timer_t* timer, void* args,
                            const struct monotime_t* time) 
{

  log_info(LD_GENERAL,"Callback activated.");

  // Cast args to our helper struct & retrieve circuit & channel from it
  delay_callback_args_t* callback_args = ((delay_callback_args_t*) args);
  circuit_t* circ = callback_args->circuit;
  channel_t* chan = callback_args->channel;
  cell_direction_t direction = callback_args->direction;
  streamid_t fromstream = callback_args->fromstream;

  // I don't know why this is done in circpad_send_padding_callback in circuitpadding.c,
  // but I'm copying it (maybe callback function just needs this signature and we're including these)
  // casts to void in order to stop the compiler from complaining?
  // (void)timer; (void)time;
  (void)time;

  cell_queue_t *queue = &circ->n_chan_delayed_cells; // change to delay queue;

  packed_cell_t* packed_cell;

  // Pop the last cell from delay queue
  packed_cell = cell_queue_pop(queue);

  // call the actual append_cell_to_circuit_queue - method
  append_cell_to_circuit_queue(circ, chan, packed_cell, direction, fromstream);

  // Free helper struct
  tor_free(callback_args);
...
/** Add <b>cell</b> to the DELAY queue of <b>circ</b> writing to <b>chan</b>
 * transmitting in <b>direction</b>.
 *
 * We firstly append to the delay queue and register a callback function
 * which transfers one cell to the regular cell queue after a delay 
 * has elapsed, i.e., the regular 'append_cell_to_circuit_queue' is called
...
void
append_cell_to_circuit_delay_queue(circuit_t *circ, channel_t *chan,
                             cell_t *cell, cell_direction_t direction,
                             streamid_t fromstream)
{

  /** IMPORTANT: we don't want to append cells that aren't relay cells
  * to the delay queue, so just call the regular 'append_cell_to_circuit_queue'-
  * method if cell command ISN'T RELAY  - OR if delays aren't activated on this circuit (8/12)
  * ... 03/03 OR if the cell direction isn't outward (strictly only introduce delays on outward direction for now)
  * ... 17/03 also apply delay for destroy cells so they cannot overtake relay cells.
  */
  // if ((cell->command != CELL_RELAY && cell->command != CELL_DESTROY) ||direction != CELL_DIRECTION_OUT || circ->delays_active != 1) {

  if (cell->command != CELL_RELAY || circ->marked_for_close ||  direction != CELL_DIRECTION_OUT || circ->delays_active != 1) {

    // need to pack the cell first because we have changed the signature of 'append_cell_to_circuit_queue'
    // to expect a packed_cell_t rather than a regular cell_t
    packed_cell_t *packed_cell = packed_cell_copy(cell, chan->wide_circ_ids);

    // and set timestamp
    packed_cell->inserted_timestamp = monotime_coarse_get_stamp();

    append_cell_to_circuit_queue(circ, chan,
                             packed_cell, direction,
                             fromstream);

  } else {

    log_info(LD_GENERAL,"Introducing a delay.");

    /** 
    *  03/03 as we have - for now - decided to only negotiate and apply delays for exitward traffic,
    *  once we've gotten to this point we know for sure that direction == CELL_DIRECTION_OUT.
    */

    cell_queue_t *queue = &circ->n_chan_delayed_cells;
    int32_t max_queue_size = max_circuit_cell_queue_size_out;
    int exitward;
    ...
    // 16/11 Do we need DoS defense for delay queue? Probably for the best (?)
    ...
    if (PREDICT_UNLIKELY(queue->n >= max_queue_size)) {
      ...
      * a client IP attacking the network. */
      ...
      log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL,
            "%s circuit has %d cells in its DELAY queue, maximum allowed is %d. "
    ...
    * this function use the stack for the cell memory. */
    ...
    /** NOT INCLUDING ALL THE OTHER HOUSEKEEPING THAT IS
    * DONE IN APPEND_CELL_TO_CIRCUIT_QUEUE, LEAVE THAT THERE 
    */

    uint8_t idx = circ->cell_counter;

    uint32_t delay_usec = circ->sampled_delays[idx];

    circ->cell_counter = ((circ->cell_counter + 1) % 32);

    // **********************************************************
    // (1) set up timer & callback

    struct timeval timeout;

    log_info(LD_GENERAL, "Delay in microseconds is %d", delay_usec);

    timeout.tv_sec = delay_usec/TOR_USEC_PER_SEC;
    timeout.tv_usec = (delay_usec%TOR_USEC_PER_SEC);

    log_info(LD_GENERAL, "timeout.tv_sec is %li", timeout.tv_sec);
    log_info(LD_GENERAL, "timeout.tv_usec is %li", timeout.tv_usec);
  
    // in the future, it would probably be elegant to have one timer per
    // circuit or sth, but to get started want to keep it really 
    // simple and avoid large re-factoring

    // Helper struct encapsulating our arguments for callback
    delay_callback_args_t* callback_args = tor_malloc(sizeof(delay_callback_args_t));
    callback_args->circuit = circ;
    callback_args->channel = chan;
    callback_args->direction = direction;
    callback_args->fromstream = fromstream;

    tor_timer_t* timer = timer_new(dequeue_from_delay_queue_callback, callback_args);
    
    // **********************************************************
    // (2) schedule timed callback
...
append_cell_to_circuit_queue(circuit_t *circ, channel_t *chan,
                             packed_cell_t *packed_cell, cell_direction_t direction,
                             streamid_t fromstream)
{
  ...
  // Set timestamp (as would be done in 'cell_queue_append_packed_copy'),
  // but our cell is already in packed form here, so we're just going
  // to update this field and append)
  packed_cell->inserted_timestamp = monotime_coarse_get_stamp();

  cell_queue_append(queue, packed_cell);
