...
// Inserted scheduled circuit reactivation ?
// ... apparently not, at least it isn't done - maybe functions marked as 'static' don't
// need to be included in '.h' files? YUP that's true :) see 
// https://stackoverflow.com/questions/42056160/static-functions-declared-in-c-header-files
// Quote: 'static functions are functions needed in only one file'

// BUT: we'll add a typedef of helper struct encapsulating the arguments provided to our callback
// function (basically, we need all the arguments required by 'append_cell_to_circuit_queue'
// because callback function calls that function and needs to pass on all its arguments)
typedef struct delay_callback_args_t {
...
} delay_callback_args_t;

void append_cell_to_circuit_delay_queue(circuit_t *circ, channel_t *chan,
                                  cell_t *cell, cell_direction_t direction,
                                  streamid_t fromstream);

void append_cell_to_circuit_queue(circuit_t *circ, channel_t *chan,
                                  packed_cell_t *cell, cell_direction_t direction,
                                  streamid_t fromstream);
