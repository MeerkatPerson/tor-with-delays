                    ...
                    crypt_path_t *cpath)
{
  ...
  if (!circ->p_streams) {
    ...
    // 8/12 also tell the circuitdelay framework
    // 21/12 do not trigger that event in circdelay framework anymore, replace by event
    // that number of cells sent on circuit are 0 mod 32

    // 01/02 uncomment for now to go back to testing static delays
    //circdelay_event_circ_has_streams(circ);
  }
