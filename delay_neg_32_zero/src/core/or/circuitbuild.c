                             ...
                             int relayed))
{
  ...
  // NOTE 17/11: replacing this call with call to our custom function
  // because although we don't want to delay CREATE cells, we need to
  // pack the cell first before being able to call append_cell_to_circuit_queue
  // as we have changed its signature to expect a packed_cell_t rather than a regular
  // cell_t.
  append_cell_to_circuit_delay_queue(circ, circ->n_chan, &cell,
