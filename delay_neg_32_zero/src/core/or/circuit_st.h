...
/** 26/11 Number of delay specs on a circuit. */
#define CIRCDELAY_MAX_SPECS (1)
...
struct circuit_t {
  ...
  /** NEW as of 16/11: cells in delay queue of n_chan */
  ...
  /** True iff this circuit has been freed */
  unsigned int has_been_freed : 1;
  ...
  /** 
  * Are delays turned on for this circuit?
  */
  unsigned int delays_active : 1;

  /**
   * 22/12 three new properties: 
   * - array holding delays (set using memcpy whenever we receive fresh delays)
   * - backup sampling seed (update every time we receive a fresh one)
   * - cell counter for determining which delay to use; increment mod 32 whenever we
   *   have relayed a relay cell
   * 03/03 moving these properties from or_circuit to circuit_st.
   */
   uint32_t* sampled_delays;
