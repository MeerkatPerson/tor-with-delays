...
circuit_close_all_marked(void)
{
  ...
  SMARTLIST_FOREACH_BEGIN(circuits_pending_close, circuit_t *, circ) {
    ...
    log_info(LD_GENERAL,"Start of SMARTLIST_FOREACH");

    tor_assert(circ->marked_for_close);

    log_info(LD_GENERAL, "Apparently no segfault when trying to access field of circuit");
    ...
    circuit_free(circ);
    // circ = NULL; // 11/11: set to NULL!!!!
  } SMARTLIST_FOREACH_END(circ);

  log_info(LD_GENERAL, "Done closing all marked circuits");
...
init_circuit_base(circuit_t *circ)
{
  ...
  cell_queue_init(&circ->n_chan_cells);
   ...
   /** 16/11 initialize delay queue as well */
  cell_queue_init(&circ->n_chan_delayed_cells);

  /** 22/12 allocate delay array */
  circ->sampled_delays = tor_malloc_zero(32 * sizeof(uint32_t));

  /** 22/12 initialize sampling seed and cell counter */
  circ->sampling_seed = 0;
  circ->cell_counter = 0;
...
circuit_free_(circuit_t *circ)
{
  ...
  /** 16/11 also clear delayed cell queue */
  cell_queue_clear(&circ->n_chan_delayed_cells);

  /** 22/12 free sampled_delays array */
  tor_free(circ->sampled_delays);
...
marked_circuit_free_cells(circuit_t *circ)
{
  ...
  /** 16/11 do the same for delayed cell queue*/
  cell_queue_clear(&circ->n_chan_delayed_cells);
